...
function cleanup_vehicles(): void {
    db_run("UPDATE vehicles SET status='offline' WHERE last_seen < DATE_SUB(NOW(), INTERVAL ? SECOND) AND status!='offline'",[VEHICLE_TIMEOUT]);
}

function haversine(float $lat1, float $lng1, float $lat2, float $lng2): float {
    $R=6371000; $a=sin(deg2rad($lat2-$lat1)/2)**2+cos(deg2rad($lat1))*cos(deg2rad($lat2))*sin(deg2rad($lng2-$lng1)/2)**2;
    return $R*2*atan2(sqrt($a),sqrt(1-$a));
}

// ── Senkrechtabstand Punkt → Segment (Meter) ──────────────────────────────────
// WICHTIG: deg2rad() ist zwingend nötig für korrekte Meterwerte!
function distToSegment(float $lat, float $lng, array $a, array $b): float {
    $cosLat=cos(deg2rad(($a[0]+$b[0]+$lat)/3)); $R=6371000;
...
//   - 10m wenn GPS auf Strasse gesnapped (Client schickt snap_lat/snap_lng)
//   - 20m als Fallback (nur rohes GPS, keine Snap-Daten)
function update_driven_segments(float $lat, float $lng, array $coords, array $driven, float $tolerance=10.0): array {
    $n=count($coords);
    for ($i=0; $i<$n-1; $i++) {
        if ($driven[$i]??false) continue;
        if (distToSegment($lat,$lng,$coords[$i],$coords[$i+1]) <= $tolerance) {
            $driven[$i]=true;
        }
    }
    return $driven;
...
function init_segments(int $numCoords): array { return array_fill(0,max(0,$numCoords-1),false); }
