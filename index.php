...
/* ── Section headers with bulk buttons ── */
...
.rc{background:var(--bg);border:1px solid var(--border);border-radius:var(--r);padding:9px 11px;position:relative;overflow:hidden;margin-bottom:6px;cursor:default;transition:opacity .2s}
...
.vd{width:7px;height:7px;border-radius:50%;background:var(--accent);flex-shrink:0;transition:background .2s}
.vd.paused{background:var(--orange)}.vd.idle{background:var(--muted)}.vd.offline{background:#333}
...
/* Smooth marker animation via CSS transition on Leaflet transforms */
...
@media(max-width:700px){
  ...
  #sidebar{width:100%;min-width:0;height:54vh;flex-shrink:0;border-right:none;border-top:1px solid var(--border);order:2}
  #mc{height:46vh;order:1}
}
</style>
</head>
<body>
...
<aside id="sidebar">
  ...
  <div class="sec-hdr" style="border-top:1px solid var(--border)">
    ...
    <div class="bulk-btns">
      <button class="bulk-btn" onclick="showAllRoutes()" title="Alle Routen einblenden">👁 Alle</button>
      <button class="bulk-btn" onclick="hideAllRoutes()" title="Alle Routen ausblenden">🚫 Keine</button>
    </div>
  </div>
  ...
  <div class="sec-hdr" style="border-top:1px solid var(--border)">
    ...
    <div class="bulk-btns">
      <button class="bulk-btn" id="btn-only-me" onclick="selectOnlyMe()" title="Nur eigenes Fahrzeug">👤 Ich</button>
      <button class="bulk-btn active" id="btn-all-veh" onclick="selectAllVehicles()" title="Alle Fahrzeuge anzeigen">👥 Alle</button>
      <button class="bulk-btn" onclick="deselectAllVehicles()" title="Alle ausblenden">🚫</button>
    </div>
  </div>
  <div id="veh-panel">
    <div id="veh-list"></div>
...
// ── Sichtbarkeit: Routen (lokal, nicht persistiert) ───────────────────────────
const localHiddenRoutes = new Set();
function isRouteVisible(route){ return route.visible && !localHiddenRoutes.has(route.id); }

function toggleRouteLocal(id){
  if(localHiddenRoutes.has(id)) localHiddenRoutes.delete(id);
  else localHiddenRoutes.add(id);
  renderRoutes(); renderRouteList();
}
function showAllRoutes(){
  localHiddenRoutes.clear();
  renderRoutes(); renderRouteList();
}
function hideAllRoutes(){
  routes.forEach(r=>localHiddenRoutes.add(r.id));
  renderRoutes(); renderRouteList();
}

// ── Sichtbarkeit: Fahrzeuge (lokal) ──────────────────────────────────────────
const deselectedVehicles = new Set(); // deselektierte Token
function isVehicleVisible(v){ return !deselectedVehicles.has(v.token); }

function toggleVehicle(token){
  if(deselectedVehicles.has(token)) deselectedVehicles.delete(token);
  else deselectedVehicles.add(token);
  updateVehBulkBtns(); renderVehicleMarkers(); renderVehicleList();
}
function selectOnlyMe(){
  deselectedVehicles.clear();
  vehicles.forEach(v=>{ if(v.token!==myToken) deselectedVehicles.add(v.token); });
  updateVehBulkBtns(); renderVehicleMarkers(); renderVehicleList();
}
...
function deselectAllVehicles(){
  vehicles.forEach(v=>deselectedVehicles.add(v.token));
  updateVehBulkBtns(); renderVehicleMarkers(); renderVehicleList();
}
function updateVehBulkBtns(){
  const allSel = deselectedVehicles.size===0;
  const onlyMe = deselectedVehicles.size===vehicles.filter(v=>v.token!==myToken).length && myToken && !deselectedVehicles.has(myToken);
...
async function loadCollections(){
  ...
  if(myToken&&myName){
    ...
    if(!r.error){
      ...
      showMapBtns(); setMyStatus('idle'); startGPS();
    }
  }
}
...
function startGPS(){
  ...
  navigator.geolocation.watchPosition(
    ...
    err=>{
      document.getElementById('gdot').className='gdot err';
      const msgs={1:'GPS verweigert',2:'Position n/a',3:'GPS Timeout'};
...
function renderRoutes(){
  routes.forEach(route=>{
    ...
    if(route.status==='pending'){
      ...
      while(i<coords.length-1){
        ...
        layers.push(L.polyline(seg,{
          color:isDriven?'#a8ff3e':'#ff4444',weight:isDriven?5:4,
          opacity:isDriven?1:0.9,dashArray:(!isDriven&&paused)?'8,5':null,lineCap:'round'
        }));
        i=j;
      }
    }
    ...
    layers.push(L.circleMarker(f,{radius:6,fillColor:route.color,color:'#fff',weight:2,fillOpacity:1,opacity:1})
      .bindTooltip('Start: '+route.name,{className:'rtt',direction:'right'}));
    layers.push(L.circleMarker(la,{radius:6,fillColor:route.status==='completed'?'#a8ff3e':route.color,color:'#fff',weight:2,fillOpacity:1,opacity:1})
...
// ── Fahrzeug-Marker mit Snap + ANIMATION ─────────────────────────────────────
function renderVehicleMarkers(){
  ...
  vehicles.forEach(v=>{
    ...
    const col=self?'#ffd700':v.status==='paused'?'#ff6b35':'#00d4ff';
    const sz=self?18:12;
    ...
    if(vehicleMarkers[v.token]){
      // ── SMOOTH ANIMATION: CSS-Transition auf dem Marker-Element ──────────
      const el=vehicleMarkers[v.token].getElement();
      if(el) el.classList.add('animated'); // aktiviert transition
      vehicleMarkers[v.token].setLatLng([dLat,dLng]);
      vehicleMarkers[v.token].setIcon(icon);
      vehicleMarkers[v.token].getTooltip()?.setContent(
        `<b>${v.name}</b>${self?' (Ich)':''}<br>${slabel(v.status)}`
      );
    } else {
      ...
      // Transition erst nach erster Positionierung aktivieren (kein Jump beim Erscheinen)
      setTimeout(()=>{
...
// ── Routen-Liste in Sidebar ───────────────────────────────────────────────────
function renderRouteList(){
  const c=document.getElementById('route-list'); c.innerHTML='';
  if(!routes.length){
    c.innerHTML='<p style="color:var(--muted);font-size:11px;font-family:var(--font-mono);padding:4px 6px">Keine Routen</p>';
    return;
  }
  routes.forEach(route=>{
    ...
    const sideCol=route.status==='completed'?'#a8ff3e':
                  (route.status==='active'||route.status==='paused')?'#ff4444':route.color;
    card.style.setProperty('--rc',sideCol);

    let acts='';
    acts+=`<button class="btn s" onclick="if(event.stopPropagation&&event.stopPropagation());document.getElementById('${route.id}_foc')&&document.getElementById('${route.id}_foc').click()" style="display:none" id="${route.id}_foc"></button>`;
    // Fokus Button
    acts+=`<button class="btn s btn-foc">🔍</button>`;
    // Sichtbarkeit (lokal)
    acts+=`<button class="btn s btn-vis" title="${visible?'Ausblenden':'Einblenden'}">${visible?'👁':'🚫'}</button>`;
    if(isJoined&&route.status==='pending')
      acts+=`<button class="btn s g btn-start">▶ Start</button>`;
    if(isJoined&&isMe&&route.status==='active')
      acts+=`<button class="btn s d btn-pause">⏸</button><button class="btn s g btn-done">✓</button>`;
    if(isJoined&&isMe&&route.status==='paused')
      acts+=`<button class="btn s g btn-resume">▶</button><button class="btn s g btn-done">✓</button>`;
    if(IS_ADMIN&&(route.status==='completed'||route.status==='paused'))
      acts+=`<button class="btn s btn-reset">↺</button>`;

    card.innerHTML=`
      <div class="rt">
        <span class="rn" style="color:${route.color}" title="${route.name}">${route.name}</span>
        <span class="rb ${route.status}">${slabel(route.status)}</span>
      </div>
...
// ── Fahrzeug-Liste in Sidebar ─────────────────────────────────────────────────
function renderVehicleList(){
  const c=document.getElementById('veh-list');
  const rm=Object.fromEntries(routes.map(r=>[r.id,r.name]));
  if(!vehicles.length){
    c.innerHTML='<div style="padding:8px 14px;color:var(--muted);font-size:11px;font-family:var(--font-mono)">Keine Fahrzeuge</div>';
    return;
  }
  c.innerHTML=vehicles.map(v=>{
    const self=v.token===myToken;
    const visible=isVehicleVisible(v);
    const col=self?'#ffd700':v.status==='paused'?'#ff6b35':v.status==='idle'||v.status==='offline'?'var(--muted)':'#00d4ff';
    return `<div class="vi${visible?'':' v-hidden'}" onclick="toggleVehicle('${v.token}')">
      ...
      <div style="flex:1;min-width:0">
        <div style="font-weight:600;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">
          ${v.name}${self?' <small style="color:var(--accent)">(Ich)</small>':''}
        </div>
        <div style="font-size:10px;color:var(--muted);font-family:var(--font-mono)">
          ${v.active_route_id?(rm[v.active_route_id]||'—'):'—'}
        </div>
      </div>
      <span class="vi-eye">${visible?'👁':'○'}</span>
    </div>`;
  }).join('');
  updateVehBulkBtns();
}
...
<style>
.rtt{background:rgba(10,12,15,.93)!important;border:1px solid #2a3340!important;color:#c8d4e0!important;
     font-family:'JetBrains Mono',monospace!important;font-size:11px!important;padding:4px 8px!important;
