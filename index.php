...
#hdr{padding:12px 16px;border-bottom:1px solid var(--border);background:#0c0e12;display:flex;align-items:center;gap:10px}
.logo{font-size:18px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--accent);flex:1}
.logo span{color:var(--text)}
.lbl{font-size:10px;letter-spacing:2px;text-transform:uppercase;color:var(--muted);
     margin-bottom:6px;font-family:var(--font-mono)}

/* Collection selector */
#col-box{padding:12px 16px;border-bottom:1px solid var(--border);background:#0d1014}
select{width:100%;background:var(--bg);border:1px solid var(--border2);color:var(--text);
       padding:8px 10px;border-radius:var(--r);font-family:var(--font-ui);font-size:14px;outline:none}
select:focus{border-color:var(--accent)}

/* Identity */
#id-box{padding:12px 16px;border-bottom:1px solid var(--border)}
#join-form{display:flex;gap:6px}
#join-form input{flex:1;background:var(--bg);border:1px solid var(--border2);color:var(--text);
  padding:8px 10px;border-radius:var(--r);font-family:var(--font-ui);font-size:14px;outline:none}
...
.vs.idle{background:#1a2030;color:var(--muted)}.vs.driving{background:#0a2010;color:var(--green)}
.vs.paused{background:#2a1a08;color:var(--orange)}

/* GPS bar */
#gps-bar{padding:7px 16px;border-bottom:1px solid var(--border);display:flex;align-items:center;
         gap:8px;font-family:var(--font-mono);font-size:11px;background:#0c0e12}
...
/* Routes panel */
#routes-panel{flex:1;overflow-y:auto;padding:10px}
#routes-panel::-webkit-scrollbar{width:3px}
#routes-panel::-webkit-scrollbar-thumb{background:var(--border2)}

.rc{background:var(--bg);border:1px solid var(--border);border-radius:var(--r);
    padding:10px 12px;position:relative;overflow:hidden;margin-bottom:7px;transition:border-color .2s}
...
.rb.pending{background:#1a2030;color:var(--muted)}.rb.active{background:#0a2010;color:var(--green)}
.rb.completed{background:#0a1a2a;color:var(--accent)}.rb.paused{background:#2a1a08;color:var(--orange)}
...
/* Buttons */
.btn{background:transparent;border:1px solid var(--border2);color:var(--text);padding:7px 12px;
     border-radius:var(--r);font-family:var(--font-ui);font-size:12px;font-weight:600;letter-spacing:1px;
     text-transform:uppercase;cursor:pointer;transition:all .2s;white-space:nowrap;text-decoration:none;display:inline-block}
...
/* Vehicles panel */
#veh-panel{border-top:1px solid var(--border);padding:10px 16px;background:#0c0e12;max-height:160px;overflow-y:auto}
.vi{display:flex;align-items:center;gap:7px;font-size:12px;padding:5px 8px;background:var(--bg);
    border:1px solid var(--border);border-radius:var(--r);margin-top:5px}
...
.mb{background:rgba(10,12,15,.85);backdrop-filter:blur(6px);border:1px solid var(--border2);
    border-radius:var(--r);padding:7px 12px;font-size:11px;font-family:var(--font-mono);pointer-events:auto}
...
#pi{position:absolute;bottom:20px;right:10px;z-index:500;display:flex;align-items:center;gap:7px;
    background:rgba(10,12,15,.85);backdrop-filter:blur(6px);border:1px solid var(--border2);
    border-radius:var(--r);padding:7px 12px;font-size:11px;font-family:var(--font-mono)}
.pd{width:7px;height:7px;border-radius:50%;background:var(--muted)}
.pd.on{background:var(--green);box-shadow:0 0 5px var(--green)}

#notifs{position:absolute;top:60px;right:10px;z-index:600;display:flex;flex-direction:column;gap:5px;max-width:260px}
.notif{background:rgba(10,12,15,.92);border:1px solid var(--border2);border-left:3px solid var(--accent);
       border-radius:var(--r);padding:9px 12px;font-size:12px;animation:si .3s ease}
...
#no-coll{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);text-align:center;
         z-index:400;color:var(--muted)}
#no-coll .big{font-size:48px;margin-bottom:12px}
#no-coll p{font-family:var(--font-mono);font-size:13px}

@media(max-width:700px){
  #sidebar{width:100%;min-width:0;position:absolute;bottom:0;left:0;right:0;
           height:52vh;border-right:none;border-top:1px solid var(--border);z-index:1000}
  #mc{height:48vh}
...
<aside id="sidebar">
  <div id="hdr">
    ...
    <?php if($is_admin): ?>
    <a class="btn s p" href="admin.php">Admin</a>
    <?php endif; ?>
    <a class="btn s" href="logout.php">Abmelden</a>
  </div>
  ...
  <div id="id-box">
    ...
    <div id="join-form">
      <input id="vname-in" type="text" placeholder="Dein Name / Team..." maxlength="30">
  ...
  <div id="gps-bar">
    ...
    <span id="gps-txt">GPS inaktiv</span>
  </div>

  <div id="routes-panel">
    <div class="lbl" style="padding:0 4px 4px">Routen</div>
...
<div id="mc">
  ...
  <div id="notifs"></div>
  <div id="no-coll" style="display:none">
    <div class="big">📦</div>
    <p>Keine aktive Sammlung<br>
    <?php if($is_admin): ?><a href="admin.php" style="color:var(--accent)">→ Im Admin-Panel erstellen</a><?php endif; ?></p>
  </div>
...
async function loadCollections() {
  const url = IS_ADMIN ? `${API}?action=collections_all` : `${API}?action=collections_active`;
  const r = await fetch(url);
  const cols = await r.json();
  const sel = document.getElementById('col-select');
  if (!Array.isArray(cols) || cols.length === 0) {
    sel.innerHTML = '<option value="">Keine Sammlung verfügbar</option>';
    document.getElementById('no-coll').style.display = 'block';
    return;
  }
  sel.innerHTML = cols.map(c => {
    const d = new Date(c.collection_date).toLocaleDateString('de-CH');
    const st = c.status ? ` [${statusIcon(c.status)}]` : '';
  ...
  currentColId = sel.value;
  if (currentColId) startPolling();
}
...
async function pollState() {
  ...
  pd.classList.add('on');
  try {
    const r = await fetch(`${API}?action=state&collection_id=${currentColId}`);
    ...
    document.getElementById('pt').textContent = 'Fehler';
  }
...
async function api(action, body={}) {
  try {
    ...
    return await r.json();
  } catch(e) { notify('Fehler: '+e.message,'w'); return {error:e.message}; }
}
...
function startGPS() {
  ...
  navigator.geolocation.watchPosition(pos=>{
    ...
    document.getElementById('gps-txt').textContent=`${lat.toFixed(5)}, ${lng.toFixed(5)}`;
    if (isJoined) api('vehicle_position',{token:myToken,lat,lng});
    if (!gpsInit) { map.setView([lat,lng],15); gpsInit=true; }
  },err=>{ document.getElementById('gps-txt').textContent='GPS: '+err.message; },
...
function renderRoutes() {
  routes.forEach(route=>{
    ...
    const c=route.color, dash=route.status==='paused'?'8,6':route.status==='pending'?'4,4':null;
    const w=route.status==='active'?5:3, op=route.status==='completed'?0.4:0.9;
    ...
    layers.push(L.circleMarker(coords[0],{radius:6,fillColor:c,color:'#fff',weight:2,opacity:1,fillOpacity:1}));
    layers.push(L.circleMarker(coords[coords.length-1],{radius:6,fillColor:route.status==='completed'?'#aaa':c,color:'#fff',weight:2,opacity:1,fillOpacity:1}));
...
function renderVehicleMarkers() {
  ...
  vehicles.forEach(v=>{
    ...
    const self=v.token===myToken, c=self?'#ffd700':v.status==='paused'?'#ff6b35':'#00d4ff', sz=self?16:12;
    const icon=L.divIcon({className:'',
      html:`<div style="width:${sz}px;height:${sz}px;background:${c};border:2px solid ${self?'#fff':'rgba(255,255,255,.7)'};border-radius:50%;box-shadow:0 0 ${self?10:6}px ${c}"></div>`,
...
function renderRouteList() {
  ...
  routes.forEach(route=>{
    ...
    card.className='rc'+(route.visible?'',' hidden-r').join('');
    if (!route.visible) card.classList.add('hidden-r');
    card.style.setProperty('--rc',route.color);
    let acts='';
    acts+=`<button class="btn s btn-tog">${route.visible?'Ausbl.':'Einbl.'}</button>`;
    ...
    if (IS_ADMIN) acts+=`<button class="btn s btn-tog2">${route.visible?'👁':'👁'}</button>`;
    card.innerHTML=`
      <div class="rt"><span class="rn" style="color:${route.color}">${route.name}</span>
        <span class="rb ${route.status}">${slabel(route.status)}</span></div>
    ...
    card.querySelector('.btn-tog')?.addEventListener('click',async()=>{
      await api('route_toggle',{route_id:route.id}); pollState();
    });
...
function statusIcon(s){return{draft:'✏️',active:'✅',completed:'✓'}[s]||s;}
...
if (myToken && myName) {
  (async()=>{
    ...
    if (!r.error) {
      ...
      document.getElementById('v-info').style.display='flex';
      document.getElementById('disp-vname').textContent=myName;
      setMyStatus('idle'); startGPS(); notify(`${myName} wieder verbunden`,'g');
    }
  })();
}
</script>
<style>
.rtt{background:rgba(10,12,15,.92)!important;border:1px solid #2a3340!important;color:#c8d4e0!important;
     font-family:'JetBrains Mono',monospace!important;font-size:11px!important;padding:5px 9px!important;
